...
T OnsetDetectionFunction<T>::complexSpectralDifference(const std::vector<T> &fftReal,
    ...
    for (int i = 0; i < fftReal.size(); i++) {
        ...
        // phase deviation
        dev = phaseVal - (2 * prevPhaseSpectrum_complexSpectralDifference[i]) +
              prevPhaseSpectrum2_complexSpectralDifference[i];
...
//===========================================================
template
class OnsetDetectionFunction<float>;

template
class OnsetDetectionFunction<double>;
